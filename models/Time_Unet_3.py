...
class RFFTM(nn.Module):
    ...
    def forward(self, x):
        ...
        # 调整阈值范围并确定K值
        energy_threshold_adjusted =  0.8 +0.1 * torch.sigmoid(self.energy_threshold)
...
class Model(nn.Module):
    ...
    def __init__(self, configs, patch_len=16, stride=8):
        ...
        self.PatchMixer_blocks = PatchMixerLayer(input_dim=self.patch_num, output_dim=self.a, kernel_sizes=self.kernel_sizes)
        self.PatchMixer_blocks1 = PatchMixerLayer(input_dim=self.d_model, output_dim=self.d_model, kernel_sizes=self.kernel_sizes)
...
# unet
    ...
    def forward(self, x_enc):
        ...
        for down_block in self.down_blocks:
            ...
            enc_out = self.Maxpools[i](enc_out)
            i = i+1
